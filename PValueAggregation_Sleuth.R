#This script details the pipeline for p-value aggregation analysis using Sleuth.
...
#Create a table mapping each transcript ID to its gene ID and name. This information is available from any kallisto abundance.tsv file.
kallisto_result <- fread("sampleID/abundance.tsv")
id_column <- kallisto_result[,1]
id_column <- cSplit(id_column, "target_id", "|")
id_column <- id_column[,c("target_id_1", "target_id_2", "target_id_6")]
colnames(id_column) <- c("target_id", "gene_id", "gene_name")
...
likelihood_test <- sleuth_lrt(model, "reduced", "full")

#Fold change can be estimated as avglogFC = log2(avgTpm(Condition samples)) - log2(avgTpm(Control samples)). 
